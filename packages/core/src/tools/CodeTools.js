    ...
    async execute(params, context) {
        try {
            const target = params.targetPath
                ? safePath(context.workspace, params.targetPath)
                : context.workspace;
    ...
    async execute(params, context) {
        try {
            ...
            if (existsSync(resolve(ws, 'package.json'))) {
                ...
                command = `go test ./...`;
                if (params.testPath)
                    command = `go test ${params.testPath}`;
                if (params.flags)
                    command += ` -v ${params.flags}`;
            }
...
export const FindImportsTool = {
    ...
    async execute(params, context) {
        try {
            const query = params.moduleOrSymbol;
            const filePattern = params.filePattern || '*.{ts,tsx,js,jsx,vue,py,go}';
            const { execSync } = await import('node:child_process');
            // 优先使用 ripgrep
            let output;
            try {
 ...
 * 搜索代码中的函数/类/变量定义
 */
export const FindDefinitionsTool = {
