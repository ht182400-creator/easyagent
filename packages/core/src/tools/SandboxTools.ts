 ...
 */
export const SandboxExecTool: ITool = {
  name: 'sandbox_exec',
...
- 只读文件系统(可选)`,
  ...
  async execute(params, context): Promise<ToolResult> {
    try {
      ...
      const image = (params.image as string) || 'node:20-alpine';
      const timeout = Math.min((params.timeout as number) || 30000, 300000);
      const readOnly = (params.readOnly as boolean) || false;
      const allowNetwork = (params.allowNetwork as boolean) || false;
      const memoryLimit = (params.memoryLimit as string) || '512m';
      const cpuLimit = (params.cpuLimit as number) || 0.5;
      ...
      // 执行命令
      const result: SandboxResult = await sandbox.exec(command, timeout);
 ...
 * 查看当前所有活跃沙箱的状态
 */
export const SandboxStatusTool: ITool = {
  name: 'sandbox_status',
 ...
 * 清理指定或所有沙箱资源
 */
export const SandboxCleanupTool: ITool = {
