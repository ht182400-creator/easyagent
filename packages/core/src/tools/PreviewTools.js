...
export const StartServerTool = {
    ...
    async execute(params, context) {
        try {
            ...
            const cwd = params.cwd ? resolve(ws, params.cwd) : ws;
...
export const DiffFilesTool = {
    ...
    async execute(params, context) {
        try {
            const ws = context.workspace;
            const file1 = params.filePath1;
            const file2 = params.filePath2;
                ...
                if (i < lines1.length && i < lines2.length) {
                    if (lines1[i] !== lines2[i]) {
                        ...
                        for (let j = blockStart; j < Math.min(i + 5, maxLen); j++) {
                            if (j >= lines1.length && j >= lines2.length)
                                break;
                            if (j >= lines1.length) {
...
//# sourceMappingURL=PreviewTools.js.map