 ...
 */
import { existsSync, statSync, renameSync, copyFileSync, mkdirSync } from 'node:fs';
 ...
 * 创建目录工具
 */
export const CreateDirTool: ITool = {
  name: 'create_dir',
  ...
  async execute(params, context): Promise<ToolResult> {
    try {
      const dirPath = safePath(context.workspace, params.dirPath as string);
 ...
 * 支持在整个项目中批量替换文本
 */
export const BatchEditTool: ITool = {
  name: 'batch_edit',
