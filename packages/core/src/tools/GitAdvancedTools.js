 ...
 * 获取当前分支名
 */
function getCurrentBranch(workspace) {
    try {
 ...
 * 生成仓库结构地图
 */
function generateRepoMap(workspace, maxDepth = 4, maxFiles = 200) {
    ...
    function walk(dir, depth, prefix) {
        if (depth > maxDepth || lines.length >= maxFiles * 2)
            return 0;
        let entries = [];
        try {
...
- 生成项目文档目录结构`,
    ...
    async execute(params, context) {
        try {
            const repoPath = params.path || context.workspace;
    ...
    async execute(params, context) {
        try {
            ...
            switch (action) {
                ...
                case 'create': {
                    if (!tagName)
                        return { success: false, content: '请指定标签名称' };
                    const args = ['tag', '-a', tagName];
                    if (params.message)
                        args.push('-m', `"${params.message}"`);
                    if (params.commitHash)
                        args.push(params.commitHash);
...
export const GitCherryPickTool = {
    ...
    async execute(params, context) {
        try {
            ...
            const noCommit = params.noCommit || false;
            if (!isGitRepo(context.workspace)) {
                return { success: false, content: '当前工作区不是 Git 仓库' };
            }
...
//# sourceMappingURL=GitAdvancedTools.js.map