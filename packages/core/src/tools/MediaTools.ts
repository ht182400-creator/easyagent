...
export const ReadImageTool: ITool = {
  name: 'read_image',
  description:
    '读取图片文件，返回base64编码的图片数据和元信息。支持 PNG/JPG/GIF/WebP/SVG 等格式。可用于视觉模型分析图片内容。',
  ...
  async execute(params, context): Promise<ToolResult> {
    try {
      ...
      if (!existsSync(filePath)) {
        return {
          success: false,
          content: `图片文件不存在: ${params.filePath}`,
      ...
      if (!SUPPORTED_IMAGE_FORMATS.has(ext)) {
        return {
          success: false,
          content: `不支持的图片格式: ${ext}。支持: ${[...SUPPORTED_IMAGE_FORMATS].join(', ')}`,
        };
      }
      ...
      if (stat.size > MAX_IMAGE_SIZE) {
        return {
          success: false,
          content: `图片过大 (${(stat.size / 1024 / 1024).toFixed(1)}MB)，限制10MB`,
        };
      }
...
export const GenerateImageTool: ITool = {
  name: 'generate_image',
  description:
    '使用AI模型根据文本描述生成图片。支持指定风格、尺寸、数量。需要配置支持视觉生成的模型提供商。',
  requiresConfirm: false,
  parameters: {
    type: 'object',
    properties: {
      ...
      size: {
        type: 'string',
        description: '图片尺寸, 如 "1024x1024", "1024x1536", "1536x1024"。默认 1024x1024',
      },
      ...
      outputDir: {
        type: 'string',
        description: '可选: 输出目录(相对于工作区), 默认 "generated-images"',
      },
    },
    required: ['prompt'],
  },
  async execute(params, context): Promise<ToolResult> {
    try {
      ...
      const outputDir = resolve(
        context.workspace,
        (params.outputDir as string) || 'generated-images',
      );
      ...
      if (!sizeMatch) {
        return {
          success: false,
          content: `无效的尺寸参数: ${size}。格式应为 "宽度x高度"，如 "1024x1024"。`,
        };
      }
...
export const ScreenshotTool: ITool = {
  name: 'screenshot',
  description:
    '对指定URL进行网页截图。需要浏览器自动化支持(Playwright/Puppeteer)。返回截图文件路径。',
  requiresConfirm: false,
  parameters: {
    type: 'object',
    properties: {
      ...
      outputPath: {
        type: 'string',
        description:
          '可选: 截图输出路径(相对于工作区), 默认 "screenshots/screenshot_<timestamp>.png"',
      },
    },
    required: ['url'],
  },
