 ...
 * 检测项目中可用的数据库连接信息
 */
function detectDatabase(workspace: string): {
  type: string;
  path?: string;
  config?: string;
} | null {
...
export const QueryDBTool: ITool = {
  ...
  async execute(params, context): Promise<ToolResult> {
    try {
      ...
      // 检测数据库
      let dbType = (params.dbType as string) || '';
      let connection = params.connection as string | undefined;
 ...
 * 列出表结构、索引、关系等
 */
export const DBSchemaTool: ITool = {
  name: 'db_schema',
          ...
          if (table) {
            ...
            if (indexes.length > 0) {
              ...
              for (const idx of indexes) {
                ...
                result += `\n  ${idx.name} (${idx.unique ? 'UNIQUE' : ''}) → ${idxCols.map((c: any) => c.name).join(', ')}`;
              }
            }
