...
/** 缓存：避免重复构建大项目的语义地图 */
let _cachedMap: ReturnType<typeof buildSemanticMap> | null = null;
let _cachedRoot = '';
let _cacheTime = 0;
 ...
 * 语义地图工具 - 构建代码库语义地图
 */
export const SemanticMapTool: ITool = {
  name: 'code_semantic_map',
  description:
    '构建代码库的语义地图，包括文件树结构、符号定义（函数/类/接口/类型/变量）、引用关系和统计信息。帮助理解大型代码库的整体结构和组织方式。支持 JS/TS/Python/Rust/Go/Java 等语言。',
 ...
 * 符号搜索工具 - 在代码库中搜索符号定义
 */
export const SymbolSearchTool: ITool = {
  name: 'code_symbol_search',
  description:
    '在代码库中搜索符号（函数、类、接口、类型等）的定义位置。支持模糊搜索和精确匹配。可用于快速定位特定功能的实现位置。',
  requiresConfirm: false,
  parameters: {
    type: 'object',
    properties: {
 ...
 * 引用查找工具 - 查找符号的所有引用位置
 */
export const ReferenceFindTool: ITool = {
  name: 'code_find_references',
  description:
    '查找指定符号在代码库中的所有引用位置。显示定义处和所有调用/使用位置，帮助理解代码的影响范围和依赖关系。',
  requiresConfirm: false,
  parameters: {
    type: 'object',
    properties: {
 ...
 * 代码库概览工具 - 快速获取项目统计信息
 */
export const CodebaseOverviewTool: ITool = {
  name: 'code_overview',
  description:
    '快速获取代码库的概览信息：文件结构树、语言分布、总行数/文件数统计。不解析符号，适合快速了解项目结构。',
  requiresConfirm: false,
  parameters: {
    type: 'object',
    properties: {
 ...
 */
export const FileStructureTool: ITool = {
  name: 'code_file_structure',
  description:
    '分析单个文件的语义结构：列出文件中所有定义的函数、类、接口、类型、变量及其位置。帮助快速了解文件内部结构。',
  requiresConfirm: false,
  parameters: {
    type: 'object',
    properties: {
  ...
  async execute(params, context): Promise<ToolResult> {
    try {
      ...
      // 按类型分组显示
      const grouped = new Map<string, typeof info.symbols>();
