...
/** 清除引擎缓存（测试用） */
export function resetBenchmarkEngine() {
    _engine = null;
}
/**
 * 加载 SWE-Bench 数据集
 */
export const LoadBenchmarkTool = {
    name: 'benchmark_load',
    ...
    async execute(params, context) {
        try {
            const dataPath = params.dataPath;
            const maxProblems = params.maxProblems || 10;
...
export const RunBenchmarkTool = {
    ...
    async execute(params, context) {
        try {
            ...
            const maxProblems = params.maxProblems || problems.length;
            const selectedIds = params.problemIds || problems.slice(0, maxProblems).map(p => p.id);
            // 创建会话
 ...
 * 查看评测结果报告
 */
export const BenchmarkReportTool = {
 ...
 * 扫描 SWE-Bench 数据
 */
export const BenchmarkScanTool = {
    name: 'benchmark_scan',
    ...
    async execute(params, context) {
        try {
            const dataDir = params.dataDir || path.join(process.cwd(), 'swebench_data');
