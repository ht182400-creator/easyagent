...
export class TelegramAdapter extends BaseIMAdapter {
    ...
    async sendMessage(chatId, text, options) {
        ...
        if (options?.inlineKeyboard) {
            params.reply_markup = {
                inline_keyboard: options.inlineKeyboard.map((row) => row.map((btn) => ({
                    text: btn.text,
    ...
    async editMessage(chatId, messageId, newText) {
        ...
        try {
            ...
            logger.warn({ error: err.message }, 'editMessage 失败');
            return false;
        }
    }
    ...
    async processMessage(msg) {
        ...
        const imMessage = {
            ...
            groupName: 'title' in msg.chat ? msg.chat.title : undefined,
            raw: msg,
        };
