...
export class WeChatAdapter extends BaseIMAdapter {
    ...
    async onStart() {
        if (!this.config.corpId || !this.config.appSecret) {
            throw new Error('企业微信接入需要配置 corpId 和 appSecret\n' +
        ...
        this.refreshTimer = setInterval(() => {
            this.refreshToken().catch((err) => {
                logger.error({ error: err.message }, '企业微信 token 刷新失败');
            });
        }, 90 * 60 * 1000);
    ...
    async sendMessage(chatId, text, _options) {
        ...
        const result = await this.fetchApi(`${WECOM_API_BASE}/message/send?access_token=${token}`, {
            method: 'POST',
    ...
    async editMessage(_chatId, _messageId, newText) {
        // 企业微信不支持编辑已发送消息
    ...
    async sendPhoto(chatId, _imageUrl, _caption) {
    ...
    async sendDocument(chatId, _fileUrl, _caption) {
        const token = await this.ensureToken();
     ...
     * @returns 回复消息 (XML 或空)
     */
    async handleMessageCallback(body) {
        // TODO: 
        // 1. 解密 XML 消息体 (WXBizMsgCrypt.decrypt)
        // 2. 解析 XML → IMMessage
        // 3. 调用 handleIncomingMessage
        // 4. 加密回复 XML → 返回
    ...
    async refreshToken() {
        ...
        const result = await this.fetchApi(url, { method: 'GET' });
