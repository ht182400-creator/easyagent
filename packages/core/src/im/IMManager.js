...
export class IMManager extends EventEmitter {
     ...
     * 消息路由核心: IM → Agent → IM
     */
    async routeMessage(adapter, message) {
        ...
        try {
            ...
            logger.error({ platform: adapter.platform, error: err.message }, '消息路由处理失败');
            await adapter.sendMessage(message.chatId, `⚠️ 处理出错: ${err.message}`);
        }
    }
    // ========== 适配器工厂 ==========
    /**
    ...
    createAdapter(config) {
        switch (config.platform) {
            ...
            default:
                throw new Error(`不支持的 IM 平台: ${config.platform}`);
        }
    }
    // ========== 查询接口 ==========
    /**
     ...
     * 获取会话映射
     */
    getSessionMapping(chatId) {
        return this.sessionMap.get(chatId);
    }
    /**
     * 设置会话映射
     */
    setSessionMapping(chatId, sessionId, platform) {
        const mapping = {
            chatId,
            sessionId,
            platform,
     ...
     * 由外部 HTTP 服务器调用
     */
    async handleWebhook(platform, req) {
        const adapter = this.adapters.get(platform);
        if (!adapter) {
        ...
        if (platform === 'feishu') {
            ...
            return feishu.handleEventCallback(req.body);
        }
        if (platform === 'wechat') {
    ...
    getWebhookPlatforms() {
        return Array.from(this.configs.entries())
            .filter(([, config]) => config.enabled && (config.platform === 'feishu' || config.platform === 'wechat'))
...
//# sourceMappingURL=IMManager.js.map