...
export class ConfigManager {
    ...
    async load() {
        try {
            ...
            this.initModelRegistry().catch((err) => {
                logger.warn({ error: err.message }, '模型目录后台更新失败');
            });
            // 加载主配置
     ...
     */
    getCurrentProvider() {
        return this.config.providers.find(p => p.id === this.config.currentModel.provider);
    }
    /**
     * 切换当前模型
     */
    ...
    setApiKey(providerId, apiKey) {
        ...
        if (provider) {
            ...
            if (preset) {
                ...
                this.config.providers.push({ ...preset, apiKey });
            }
            else {
