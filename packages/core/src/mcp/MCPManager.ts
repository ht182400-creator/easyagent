...
import type { MCPServerConfig, MCPTool, ToolResult, ToolContext } from '../types/index.js';
...
export class MCPManager {
  ...
  private mcpToIToolMap = new Map<string, string>(); // MCP工具名 → 带前缀的ITool名

  /** 连接 MCP 服务器 */
  async connect(config: MCPServerConfig): Promise<ITool[]> {
  ...
  /** 调用 MCP 工具 */
  async callTool(fullName: string, params: Record<string, unknown>): Promise<ToolResult> {
  ...
  private convertToITool(mcpTool: MCPTool, config: MCPServerConfig): ITool {
