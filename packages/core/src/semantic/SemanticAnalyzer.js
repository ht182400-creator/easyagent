...
/** 检查路径是否应该被忽略 */
function shouldIgnore(filePath, root) {
 ...
 * 递归收集目录中所有源文件
 */
export function collectSourceFiles(root, maxDepth = 8) {
    const files = [];
    function walk(dir, depth) {
        if (depth > maxDepth)
            return;
        if (shouldIgnore(dir, root))
            return;
        let entries;
        try {
 ...
 * 构建完整语义地图
 */
export function buildSemanticMap(rootPath, maxDepth = 8, maxFiles = 500) {
    ...
    for (const filePath of limitedFiles) {
        ...
        for (const symbol of info.symbols) {
            if (['import', 'export'].includes(symbol.kind))
                continue;
            // 符号索引
 ...
 * 搜索符号定义
 */
export function searchSymbol(map, query, caseSensitive = false) {
    const results = [];
 ...
 * 查找符号的所有引用位置
 */
export function findReferences(map, symbolName, workspaceRoot) {
    const references = [];
    ...
    for (const file of map.files) {
        try {
            ...
            while ((match = regex.exec(content)) !== null) {
                ...
                // 跳过已在符号索引中的定义
                const isDefined = map.symbolIndex.get(symbolName)?.some(s => s.filePath === file.filePath && s.line === line);
                if (isDefined)
                    continue;
                references.push({
                    symbol: symbolName,
 ...
 * 生成语义地图的文本表示
 */
export function formatSemanticMap(map) {
    const lines = [
        `📊 代码库语义地图`,
        `根目录: ${map.root}`,
        ``,
    ...
    if (map.symbolIndex.size > 0) {
        ...
        for (const [name, syms] of sortedSymbols) {
            const locations = syms.map(s => `${path.relative(map.root, s.filePath)}:${s.line}`).join(', ');
 ...
 * 生成ASCII文件树
 */
function generateFileTree(root, files, maxDepth) {
    const lines = [];
 ...
 */
export function getCodebaseOverview(rootPath) {
    const root = findRepoRoot(rootPath);
