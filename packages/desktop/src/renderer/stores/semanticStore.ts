...
export const useSemanticStore = create<SemanticState>((set) => ({
  ...
  fetchSemanticMap: async (path, depth, maxFiles, refresh) => {
    ...
    try {
      ...
      const data = await apiFetch<any>(`/api/semantic/map?${params}`);
  ...
  searchSymbols: async (query, path, caseSensitive, kind) => {
    ...
    try {
      ...
      const data = await apiFetch<any>(`/api/semantic/search?${params}`);
  ...
  findReferences: async (symbol, path) => {
    ...
    try {
      ...
      const data = await apiFetch<any>(`/api/semantic/references?${params}`);
  ...
  fetchOverview: async (path) => {
    ...
    try {
      ...
      const data = await apiFetch<any>(`/api/semantic/overview?${params}`);
  ...
  analyzeSingleFile: async (filePath) => {
    ...
    try {
      const data = await apiFetch<any>(`/api/semantic/file?${params}`);
