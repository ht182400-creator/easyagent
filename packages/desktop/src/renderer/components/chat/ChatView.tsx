...
import { useSessionStore, type ChatMessage, type ToolCall } from '@/stores/sessionStore';
              ...
              <div className="mt-2 space-y-1.5">
                {msg.toolCalls.map((tc) => (
                  ...
                  >
                    ...
                    {tc.error && (
                      <p className="mt-1 text-red-400">{tc.error}</p>
                    )}
                  </div>
                ))}
              </div>
...
export const ChatView: FC<ChatViewProps> = ({ sessionId }) => {
  ...
  const { messages, addMessage, appendToLastMessage, setLastMessageStreaming, clearMessages, isRunning, setIsRunning, addToolCall, updateToolCall } = useSessionStore();
  ...
  const handleSend = useCallback(
    ...
    [input, isRunning, addMessage, appendToLastMessage, setLastMessageStreaming, setIsRunning, addToast, updateTabTitle, tabs],
  );
  ...
  const handleStop = useCallback(() => {
    ...
    setIsRunning(false);
    setLastMessageStreaming(false);
  }, [setIsRunning, setLastMessageStreaming]);
