...
import { ConfigManager, SessionManager, ToolRegistry, AgentEngine, getAllBuiltinTools, } from '@easyagent/core';
/** 全局计数器 */
let msgCounter = 0;
...
export const App = ({ initInfo }) => {
    ...
    // ============ 消息操作 ============
    /** 添加消息 */
    const addMsg = useCallback((role, content, streaming = false) => {
        const id = nextId();
        setMessages((prev) => [...prev, { id, role, content, isStreaming: streaming }]);
        return id;
    }, []);
    /** 追加到末条消息(流式输出) */
    const appendLast = useCallback((content) => {
        setMessages((prev) => {
            const msgs = [...prev];
    ...
    // ============ 命令处理 ============
    const handleCommand = useCallback(async (rawCmd) => {
        ...
        switch (action) {
            ...
            case 'models': {
                try {
                    ...
                    const modelList = config.providers
                        .flatMap((p) => (p.models || []).map((m) => `  ${p.name}: ${m.id}`))
                        .join('\n');
                    addMsg('system', `可用模型:\n${modelList || '  无'}`);
                }
                catch {
            ...
            case 'providers': {
                try {
                    ...
                    const pList = config.providers
                        .map((p) => `  [${p.hasKey ? '✓' : '✗'}] ${p.name} (${p.id}) - ${p.models?.length || 0} 模型`)
                        .join('\n');
                    addMsg('system', `提供商:\n${pList}`);
                }
                catch {
            ...
            case 'sessions': {
                try {
                    ...
                    const sList = sessions
                        .map((s) => `  ${s.id?.slice(-12)} - ${s.title || '未命名'} [${s.status || 'active'}]`)
                        .join('\n');
            ...
            case 'tools': {
                ...
                addMsg('system', `工具 (${tools.length}):\n${tools.map((t) => `  ${t.name.padEnd(24)} ${t.description?.slice(0, 50)}`).join('\n')}`);
                break;
            }
            case 'token-key': {
                if (args.length >= 2) {
    ...
    // ============ 渲染 ============
    return (<Box flexDirection="column" padding={1} height="100%">
      {/* 欢迎 Banner */}
      {showBanner && messages.length <= 1 && (<Banner version="v0.5.0"/>)}
      ...
      {/* 状态栏 */}
      <StatusBar state={agentState} model={initInfo.model} toolCount={initInfo.tools} tokenUsage={tokenUsage}/>
    </Box>);
};
