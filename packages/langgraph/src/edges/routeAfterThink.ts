 ...
 */
export function routeAfterThink(state: typeof AgentState.State): 'act' | '__end__' {
  ...
  if (getMessageType(lastMessage) === 'ai' && hasToolCalls(lastMessage)) {
    const toolCalls = (lastMessage as unknown as Record<string, unknown>).tool_calls as Array<{
      name: string;
    }>;
    log.decision('路由 → act', {
