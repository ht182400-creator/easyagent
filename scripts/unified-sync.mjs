...
try {
  ...
  const { parseMemoryIssues } = await import(
    pathToFileURL(resolve(PIPELINE_DIR, 'lib/pipeline-parser.mjs')).href
  );
  issueResult = parseMemoryIssues(memoryDir, cacheFilePath);
  log.info(
    `已解析: ${issueResult._totalIssues} 个问题, ${Object.keys(issueResult.modules).length} 个模块`,
  );

  // 生成问题摘要列表（供仪表板使用）
  for (const [mid, mod] of Object.entries(issueResult.modules)) {
    if (mod.issues && mod.issues.length > 0) {
      const pendingCount = mod.issues.filter(
...
const scoreHistory = [
  ...
  { version: 'v0.5.1', date: '2026-06-25', score: 96, note: '分支10项+集成测试106' },
  {
    ...
    date: '2026-06-25',
    score: 100,
    note: `${Object.keys(MODULE_REGISTRY).length}/${Object.keys(MODULE_REGISTRY).length}模块+${totalTestCases}用例`,
  },
];
...
// dashboard 分离为独立文件 (#11 精简：pipeline-data.json 从 330KB → ~5KB)
const dashDataPath = resolve(PIPELINE_DIR, 'dashboard-data.json');
writeFileSync(dashDataPath, JSON.stringify(rawDashboard, null, 2), 'utf-8');
writeFileSync(pdPath, JSON.stringify(pipelineData, null, 2), 'utf-8');
log.ok('pipeline-data.json + dashboard-data.json 已更新:');
log.info(`  阶段数: ${phases.length}, 分支数: ${branches.length}`);
log.info(
  `  节点数: ${phases.flatMap((p) => p.nodes).length + branches.flatMap((b) => b.nodes).length}`,
);
log.info(`  KPI: ${mappingKPI.testCases} 用例, ${mappingKPI.scoreTotal}/100 分`);
log.info(`  Dashboard: ${Object.keys(rawDashboard).length} 个面板 (→ dashboard-data.json)`);
...
const withoutTests = allModIds.filter(
  (id) => !MODULE_REGISTRY[id].testFiles || MODULE_REGISTRY[id].testFiles.length === 0,
);
...
log.ok(
  `测试用例覆盖率: ${totalTestCases - utilsCases}/${totalTestCases} (${Math.round(((totalTestCases - utilsCases) / totalTestCases) * 100)}%)`,
);
