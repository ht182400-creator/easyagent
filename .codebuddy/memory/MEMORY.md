...
- **Node**: ≥18 可装；**推荐 20/22 LTS**。Node ≥24 = **警告放行**（2026-09-19 起，原硬拦已撤：better-sqlite3@12 无 Node24 预编译，但有工具链会自动编译，或设 `EASYAGENT_SQLITE_DRIVER=node` 免编译；`EASYAGENT_SKIP_NODE_CHECK` 已无作用）；启动前 `node scripts/sqlite3-loader.mjs system`
- **SQLite 驱动约束（2026-09-19 核实）**: **已加适配层** `packages/core/src/db/sqlite.ts`（统一入口 `openDatabase`，默认 better-sqlite3，`EASYAGENT_SQLITE_DRIVER=node` 切内置 `node:sqlite`；测试已**不再用假库 mock**，两个驱动由参数化测试守护）；**桌面端已切 Electron 35.7.5 + 内置驱动**（better-sqlite3/bindings/file-uri-to-path 已从 desktop 依赖移除，`main.ts` 设驱动 env；**EXE 真实打包+安装验证待人工**，electron-builder 仍精确锁 23.6.0）· 默认 `better-sqlite3@^12.11.1`（Node 18/20/22 有预编译；**Node 24 本地编译后实测可用**，ABI 137）· `node:sqlite` 需 **Node ≥22.5** 且官方仍是 **Stability 1.2 RC**（v22.5 加入，v25.7 升 RC，从未 Stable）· **Electron 30 = Node 20.11.1** → 桌面端用不了 `node:sqlite`（**分界：Electron 35 = Node 22.14.0 起才有**；Electron 34 = Node 20.18.1 仍无）· better-sqlite3 **13.x 的 engines 是 `node >= 22`**（升它会把下限抬到 22 并连带要求 Electron ≥35）· 桌面若升 Electron 还需连带升级 `electron-builder`（现为 23.6.0，太老）并复验打包链路
