...
| **语义分析「静默 0」（cwd 陷阱）** | `/api/semantic/*` 未传 path 时曾退回 `process.cwd()`（Electron 下是安装目录甚至 System32）→ 扫 0 文件却返回 `success:true` + 全 0 统计，前端只显示四张 0 卡，**用户无法自查** | 默认目录必须用**注入的 `projectRoot`**（Web=仓库根 / Desktop=宿主传入）；显式 path 非法要回 400；空结果要在 UI 回显「本次扫描根」（`routes/semantic.ts` 的 `resolveWorkspace`）。F14 先例 |
| **语义扫描会被日志/文档灌满** | `logs/**`（每次回归产出的 json/html）未被忽略 + `maxFiles` 是硬上限 → 300 个名额被非代码文件占满，实测**本仓库 typescript 文件数 = 0** | `IGNORE_PATTERNS` 已含 `logs`/`release`；`prioritizeSourceFiles()` 保证**代码文件优先占满** maxFiles（新增语义扫描相关逻辑前先看该函数）。F28 先例 |
| **格式债务（prettier）** | 252 个文件不符合 prettier → CI 的 Format check 只能 `continue-on-error`，门禁形同虚设 | **已于 2026-09-19 清零**：提交前跑 `pnpm format`，保持 `prettier --check "packages/**/*.{ts,tsx,json,css}" "scripts/**/*.mjs" "docs/**/*.md"` 全绿（`.prettierignore` 已排除 node_modules/dist/lockfile） |
| **3000 端口已被自建 Forgejo 占用** | EasyAgent 主服务 = **3456**（`PORT`）、前端 Vite = 5173、LangGraph Demo = 3455；但预览工具曾默认 3000 → 与 Forgejo 撞（EADDRINUSE）。安装脚本也曾提示 3000（错，实为 3456） | **禁止把 3000 当默认端口**；用户项目预览用 **3500-3599 自动选空闲**（`PreviewTools.findAvailablePort`）；端口权威表 = `docs/52`。F15 先例 |
| **语义地图 `maxFiles` 只是"天花板"** | 开销 ∝ **实际文件数**而非上限（cap=1000/3000/10000 实测均 ~630ms）；超出上限的文件会被截断（代码优先排序后 `slice`） | 默认 **1000**（本仓 716 文件全覆盖；旧 300 会漏 216 个代码文件）；界面已暴露「深度/文件上限」+「强制重扫」；截断由 `stats.totalCandidates`/`truncated` 暴露 + UI 告警条 —— **禁止再静默截断**（同"静默 0"家族缺陷）。F29/F30 先例 |
| **语义扫描有三级缓存，改动后必须接 force 路径** | ① 路由 SWR（60s，key=`workspace\|depth\|maxFiles`）② Worker 内地图缓存（60s）③ core **两级增量缓存**：`analysisCache`（符号分析）+ `lineCountCache`（概览行数），指纹均 = `mtimeMs + size`，构建后裁剪到本次扫描集合、上限 5000 条 | `refresh=true` 依次清：主线程 `resetSemanticCache()` + `clearAnalysisCache()`（两者都清）、Worker 内 `clearAnalysisCache()`（见 `buildWorkerSource` 的 `getMap`）。**新增缓存必须接入 force**，否则"强制重扫"名不副实（曾踩：refresh 复用旧结果）。F30/F31 先例 |
| **语义扫描性能画像：瓶颈是 I/O 不是 CPU（勿凭直觉优化）** | 剖析（717 文件/35MB）：**纯读盘 243ms(59%) > 正则解析 150ms(36%) > 遍历 5%**。已治：Buffer 直读（行数按字节、`buf.length` 免再编码、无符号模式语言 md/json 不解码）、并发读盘 `buildSemanticMapAsync`/`getCodebaseOverviewAsync`、忽略模式预编译 | **已证伪的伪优化**：`UV_THREADPOOL_SIZE`、提高 `concurrency` 都在噪声内（本机抖动 ±30%）→ 不要重走；**并发只对"纯 I/O"的概览显著**（618→213ms），地图 367→257ms。评测必须**多次取中位数**，且并发版与同步版结果必须一致（有测试守护）。**剩余空间 = 正则解析 ~150ms**（候选：worker_threads 分片解析 / tree-sitter / indexOf 预筛；已登记 `docs/44` #19，做前先补解析基准）。F31 先例 |
| **沙箱命令黑名单会误杀引号内字符** | 旧 `containsShellMetacharacters` 把引号内的 `(` `)` 当子 shell → `node -e 'console.log(1+1)'` 被拒；且 exec 超时被 kill 后 `if (killed) return` → Promise 永不 settle（请求挂死） | 已根治为**不经 shell**（`spawn(argv, {shell:false})` + `sandbox/commandSafety.ts` 引号感知解析）；改沙箱执行逻辑前先读该模块头注释。F17 先例 |
| **`checkDockerAvailability` 是一次性闩锁缓存** | `dockerChecked = true` 后**永久**返回旧结论（无 TTL）→ Docker 在后端启动之后才可用时，沙箱一直停在本地模式、`/status` 也刷不出新状态、UI「重新检测」按钮无效 | 需要重探先调 `resetDockerCache()`（已从 core 根入口导出）；`SandboxManager.createSandbox` 已有 30s 节流自愈，`/api/sandbox/status` 已改为每次实探。F21 先例 |
| **ANSI 颜色码不要写进日志文件** | ANSI 是**字节**，只有终端会渲染成颜色；IDE 日志面板/浏览器/编辑器里只会变成「小方块 + `[39m`」噪音（用户实报） | 落盘/进 HTML 前必须 `stripAnsi()`；子进程用 `NO_COLOR=1` + `FORCE_COLOR=0` 抑制（`run-tests-log.mjs` 已实现，勿在日志链路里重新引入颜色） |
| **`{ type:'信封', ...event }` 会被 event.type 覆盖** | WS 广播自动化进度时写 `{ type:'automation_progress', ...event }`，而 event 带 `type:'agent_start'` → **信封类型被覆盖**，前端只认信封类型 ⇒ 实时进度日志永远为空（用户实报：只显示「执行中」） | 信封字段必须放在展开**之后**（或另起 `step` 字段）；前端用 `normalizeAutomationStep()`（`step ?? type`，两种形态都兼容）。F24 先例 |
| **`ask_user` 在自动化里无人应答** | 无人值守任务调用 `ask_user` 时旧实现只把问题当文本返回，Agent 只能自己猜 | **已支持 HITL**：服务端注册「可交互版」`ask_user`（`ToolRegistry.register()` **按名覆盖**，两引擎通用，无需改 Agent 内核）→ `server/src/hitl.ts` 的 `QuestionBroker` 广播 `agent_question`，用户在界面上答；超时 5 分钟则让 Agent 自行决策。新增接口 `GET /api/agent/questions`、`POST /api/agent/answer`。F25 先例 |
| **前端缺 import 只有 tsc 能发现** | ESM 下用了未导入的标识符（如 JSX 里 `<RefreshCw />` 忘 import）= **运行时 `ReferenceError` → 整页黑屏**；esbuild/vite 构建**不检查**，IDE 诊断也可能滞后（本轮就漏报过） | 已固化门禁：`pnpm verify:types`（= `scripts/verify-frontend-types.mjs`，也在 `verify:all` 的 `types` 项里，前端已 0 错误）；CI 侧 `_test.yml` 的 test-desktop / build-check 早有 `npx tsc --noEmit`。vitest 不做类型检查，**不能当门禁** |
| **测试写用户真实数据目录（自我毒化）** | ① `KnowledgeService.getGlobal()` 默认 `~/.easyagent/.easyagent/knowledge`，core/server 并行写它 → 互污；用例失败后清理代码不执行 → **永久失败** ② `AutomationManager` 默认 `~/.easyagent/data/automations.json`，服务端测试每跑一次回归留 2 条任务 → 累积成「任务列表 100 条」（用户实报） | 测试必须重定向：知识库 `EASYAGENT_GLOBAL_KB_DIR`、自动化 `EASYAGENT_AUTOMATIONS_FILE`（写入临时文件）；`addDocument` 同名复用旧 id。F18 / F23 先例 |
...
- **⚠️ 2026-09-19 实测基线（勿再用旧数字）**: 定义用例 **1798**（模块映射口径，真源 `docs/pipeline/test-case-mapping.json`）；**Vitest 已执行 1809，全部通过，0 失败**。**历史值 1195 / 1260 / 1514 / 1561 / 1572 / 1624 / 1635 / 1629 / 1640 / 1664 / 1675 / 1661 / 1672 / 1669 / 1680 / 1685 / 1696 / 1699 / 1710 / 1715 / 1718 / 1726 / 1729 / 1747 / 1753 / 1758 / 1761 / 1784 / 1785 / 1787 / 1794 / 1796 / 1805 均已过期**。真源 = `docs/pipeline/test-case-mapping.json`，由 `node scripts/verify-data-consistency.mjs` 作为 CI 门禁校验（见 §12）
