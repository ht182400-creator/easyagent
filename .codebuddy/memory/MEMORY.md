...
- **版本**: v0.5.0 (1052 test cases, 1051 passed, 1 skipped, 覆盖率门禁已配置, CI 6/6 jobs)
...
```bash
cd packages/core && npx vitest run     # 814 tests (JSON → _vitest-core.json)
cd packages/server && npx vitest run   # 45 tests (JSON → _vitest-server.json)
...
**当前结果**: 893 用例 / 892 通过 / 1 skipped / 99.9% pass rate。
...
- 测试文档: `docs/03_测试案例文档.md` (893 tests, 覆盖率门禁)
