...
> 版本: v1.8 | 日期: 2026-06-26 | 基于 10+ 次打包实战经验总结 | 28 个问题已解决 | 当前版本: 0.6.6
...
```bash
# ============ 推荐：项目根目录一键脚本（双击） ============
build.bat              # 快速测试模式 (--dir, ~60s)
build.bat --release    # 完整安装包 (NSIS, ~3min)
...
3. **构建**: build-shared.bat (core + server) → desktop tsup → vite build
...
| 2   | **package.json 配置** | electron-builder 精确 23.6.0 (非 ^)、electron 精确 35.7.5 (非 ^)，无 v24 残留，npmRebuild:false                   | 🔴 致命 |
...
| 6   | **关键依赖**          | express, ws, cors, multer, pino, body-parser 已声明（**不再含 better-sqlite3/bindings**：桌面用内置 node:sqlite） | 🟡 影响 |
| 7   | **CSS @import**       | index.css 中 @import 在所有规则之前                                                                               | 🟡 影响 |
| 8   | **数据库驱动**        | main.ts 设置 `EASYAGENT_SQLITE_DRIVER=node`；`verify-build.cjs` 应输出 "no native dep"                            | 🔴 致命 |
...
build.bat 执行流程:
  ...
  ├── [2/5] build-shared.bat: core tsup + server tsup (与 build-web.bat 共用)
  ├── [3/5] desktop tsup + vite build
  ├── [4/5] sqlite3 版本切换 (System → Electron)
...
export default defineConfig({
  ...
  external: [
    ...
    'better-sqlite3', // 原生模块
    'pino',
    'pino-pretty', // CJS require
    'express',
    'cors',
    'ws',
    'multer', // Express 生态
    'electron-updater',
    'electron-store', // Electron 插件
  ],
...
<html lang="zh-CN">
  <head>
    ...
    <meta
      http-equiv="Content-Security-Policy"
      content="default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; connect-src 'self' http://127.0.0.1:3456 ws://127.0.0.1:3456; img-src 'self' data:; font-src 'self' data:;"
    />
...
| 2026-09-19             | —     | 23.6.0（未动）   | ⏳ **打包验证待做** | **Electron 30 → 35.7.5**（探针实测：Node 22.16 / ABI 133 / `node:sqlite` 可用）；桌面改用内置驱动并**移除 better-sqlite3/bindings/file-uri-to-path**（删 `postinstall.cjs` 双版本管理、`verify-build.cjs` 重写为"应无原生模块"检查）；代码/配置/单测（215/215）已验证，**EXE 真实打包与安装仍需人工跑一次**（electron-builder 23.6.0 对 Electron 35 的兼容性未证实，若失败则升级 builder 并整链复验） |
