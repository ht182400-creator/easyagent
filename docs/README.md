...
```
MEMORY.md (项目根目录 .codebuddy/memory/)  ← 陷阱清单 62 条 + 高频问题速查
修复汇总.md                                 ← 🆕 所有修复汇总的集中记录，按日期查找
35_MODULE_VERSION问题分析与根治方案.md     ← better-sqlite3 必读（服务端；桌面已内置 node:sqlite）
...
|    `03`    | 测试案例文档                                             | 1578 用例 → **现 1847 定义用例（已执行 1868）**，覆盖率报告，含 Phase D   |
...
**最常踩的坑**（完整 62 条见 `MEMORY.md` → 关键陷阱清单）：
